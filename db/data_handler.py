...
class DataHandler():
    ...
    def __init_table(self) -> dict:
        # ndarray table
        if (self.db.is_table_exists("ndarray") == False):
            self.db.create_table("ndarray", ["id", "data", "shape"])

        # object table
        if (self.db.is_table_exists("object") == False):
            self.db.create_table("object", ["id", "data", "info"])
        
        # config table
        if (self.db.is_table_exists("config") == False):
    ...
    def save_numpy(self, dict_str:str, ndarray:np.ndarray, is_force:bool=False, split_str:str = ".") -> bool:
        ...
        if (self.__check_keys_and_insert(data_mapper, keys, data_id_ptr + 1, dict_str, is_force) == True):
            # Save the ndarray
            out = io.BytesIO()
            np.save(out, ndarray)
            out.seek(0)
    ...
    def save_obj(self, dict_str:str, obj:object, info:str="", split_str:str = ".", is_force:bool=False) -> bool:
        keys = dict_str.split(split_str)
